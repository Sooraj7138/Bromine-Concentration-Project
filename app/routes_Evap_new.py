def register_evaporation_routes(app):
    from flask import render_template, request, redirect, url_for, send_file, flash
    import os
    from Evaporation_service import forecast_future_days
    ...
    @app.route('/Evaporation', methods=['GET','POST'])
    def Evaporation():
        return render_template('Evaporation.html')

    @app.route('/EvaporationRate', methods=['GET', 'POST'])
    def EvaporationRate():
        if request.method == 'POST':
            ...
            try:
                ...
                evaptype = request.form.get("EvapCategory")

                prediction_df, plot_html = forecast_future_days(user_start, user_end, mode=mode, evaptype=evaptype)
                ...
                prediction_df.to_csv(
                    "D:/Sooraj/Project_Bromine_Concentration/app/outputs/evaporation_prediction_output.csv", index=False)
                ...
                return redirect(url_for('Evaporation'))
    ...
    @app.route("/download_predictions_Evap")
    def download_predictions_Evap():
        try:
            file_path = "D:/Sooraj/Project_Bromine_Concentration/app/outputs/evaporation_prediction_output.csv"

            if not os.path.exists(file_path):
                flash("❌ No predictions available to download.")
                return redirect(url_for("EvaporationRate"))

            return send_file(
                file_path,
                mimetype="text/csv",
                as_attachment=True,
                download_name="Evaporation_Prediction.csv"
            )
