...
DATA_PATH = os.path.join(BASE_DIR, 'data', 'Trainer.csv')
OUTPUT_PATH = os.path.join(BASE_DIR, 'outputs', 'trainer_prediction_output.csv')

FEATURES = ['Day', 'Month', 'Year', 'Month_sin', 'Month_cos']
TARGET = 'Final_Concentration_(gpl)'
MODEL_NAME = 'xgb_trainer_model'
...
def train_model(df_clean, y_column, model_name, scaler_x):
    ...
    # Train XGBoost model
    model = XGBRegressor(n_estimators=200, learning_rate=0.20, max_depth=5, subsample=0.8, colsample_bytree=0.8)
    model.fit(X_train, y_train.ravel())

    # Save model and scaler
    model_path = os.path.abspath(os.path.join(MODEL_DIR, f"{model_name}_trainer_wl.h5"))
    joblib.dump(model, model_path)
    joblib.dump(scaler_y, os.path.abspath(os.path.join(MODEL_DIR, f'{model_name}_trainer_scaler_wl.pkl')))
...
def train_C2_model():
    ...
    joblib.dump(scaler_x, os.path.join(MODEL_DIR, 'x_trainer_scaler_wl.pkl'))
    joblib.dump(FEATURES, os.path.join(MODEL_DIR, 'x_trainer_features_wl.pkl'))
    ...
    joblib.dump(feature_ranges, os.path.join(MODEL_DIR, 'x_trainer_feature_ranges_wl.pkl'))
    print("✅ Saved input feature value ranges for validation.")

# === Forecast Function ===
def forecast_future_C2(start_date_str, end_date_str):
    ...
    model = joblib.load(os.path.join(MODEL_DIR, f'{MODEL_NAME}_trainer_wl.h5'))
    scaler_y = joblib.load(os.path.join(MODEL_DIR, f'{MODEL_NAME}_trainer_scaler_wl.pkl'))
    scaler_x = joblib.load(os.path.join(MODEL_DIR, 'x_trainer_scaler_wl.pkl'))
    feature_ranges = joblib.load(os.path.join(MODEL_DIR, 'x_trainer_feature_ranges_wl.pkl'))
    ...
    fig.update_layout(
        title='Predicted Final Concentration (gpl)',
        xaxis_title='Date',
    ...
    df_future['Final Concentration (gpl)'] = df_future[TARGET]
    return df_future[['Date', 'Final Concentration (gpl)']], plot_html
